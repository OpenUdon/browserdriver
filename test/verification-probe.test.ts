...
test("v7 accepts only complete verification-only requests before browser launch", async()=>{
 ...
 for(const change of [{version:"udon.browser-driver.v8"},{type:"register"},{type:"close"},{credentialBindings:{}},{input:{}},{profile:null},{allowedOrigins:[null]}]) assert.throws(()=>parseInput(JSON.stringify({...request,...change})),DriverFailure);
});
