...
export async function runProviderFixture(provider: Provider, activation: Activation,
  ...
  try {
    ...
    guard = new VerificationGuard(context, verificationDescriptor(provider, activation, origin), new Set([origin]),
      new Set([origin + "/ready", origin + "/register", origin + "/complete"]), operationDeadline, false, true, true);
    await guard.install();
  ...
  return {version: "browserdriver.provider-fixture.v6" as const, chromiumSandbox: true,
