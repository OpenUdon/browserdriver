...
Status: [M13](status-M13.md).

M13.4 is active after all three separately authorized official-key fixtures
failed against the adopted synthetic closure. The approved successor adds
value-free diagnostics, independent mode selection and demonstrated fixes;
focused local checks precede a concrete bounded provider retest proposal.
Preserve the original failures and adopted kit. Renew complete qualification
only after provider integration passes. See the appended M13 record.

The separately authorized six-case successor retest is now complete: one pass
(reCAPTCHA approved_submit), five failures (Turnstile script redirects, hCaptcha
provider-frame checks, reCAPTCHA before-approval readiness timeout). Teardown
and evidence bindings pass for every case. M13.4 remains blocked on these
integration findings; both sets of consumed claims and the frozen kit remain
unchanged. See the latest diagnostic evidence in the M13 status file.

The reCAPTCHA before-approval timeout now has owner-confirmed context: the
window was visible and its checkbox was not completed. This is consistent with
waiting for human input, while that mode's readiness remains unverified. The
result and claim stay consumed; the separate redirect/frame and shutdown
diagnostic findings remain open.
...
| M15 | [status-M15.md](status-M15.md) | M15.1 protocol and M15.2 synthetic count execution complete; M15.3 full gate pending; Browsertools M32 is published |
...
### Authorized redirect/frame/shutdown successor — September 13

The owner requests local repairs for Turnstile redirects, hCaptcha frame
handling and late shutdown diagnostics, plus an explicit human readiness
confirmation before the next fixture verification countdown. Browserdriver
M13.5 owns implementation, offline regressions, one affected fresh synthetic
smoke and bounded review. W16.4i coordinates this work; no provider pair or
registration attempt is reopened. The previous candidate, adopted runtime and
all consumed reports remain immutable. Provider re-execution, publication,
full qualification and adoption remain separate gates.


September 13 M13.5 local closure: reviewed redirect/frame/shutdown repairs and
fixture readiness confirmation pass 72 offline tests, both affected fresh
synthetic Chromium smokes, audit and W8M fast checks. Bounded review iteration 2
has no open P1/P2. Diagnostics and fixture reports are v2; driver v6/UWS 1.2
remain unchanged. Prior consumed attempts and frozen/adopted bytes are intact.
A new immutable fixture candidate/supervisor scope and fresh provider authority
are next; real-provider acceptance and renewed qualification/adoption are pending.
...
September 13 fresh six-case closeout: M13.6/W16.4i records three passes
(reCAPTCHA before approval and both hCaptcha modes) and three failures (both
Turnstile response-type checks and reCAPTCHA after invisible activation).
All 78 observed process identities/listeners closed without force; three local
POSTs were discarded. Scoped review passes; every invocation is consumed.
Turnstile redirects and hCaptcha frames advanced, but provider integration,
renewed qualification/adoption and W8M acceptance remain incomplete. Evidence
is in `w16-provider-retest-v2-20260913`; prior adopted bytes and results are intact.
